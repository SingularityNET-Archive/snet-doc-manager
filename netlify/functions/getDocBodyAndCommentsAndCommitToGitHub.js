...
async function commitDocumentTextAndCommentsToGitHub(docs, date = null) {
    const octokit = new Octokit({ auth: process.env.GITHUB_TOKEN });
    for (const doc of docs) {
      const documentTextAndComments = await getDocumentTextAndComments(doc, date);
      ...
      if (documentTextAndComments.includes('> [Comments]')) {
        let path;
        if (date) {
          path = `Data/${doc.entity}/Content/${doc.workgroup}/Docs/GoogleDocs/${doc.originalDocId}/Archive-copies/${date}/${doc.google_id}/doc-with-comments.md`;
        } else {
          path = `Data/${doc.entity}/Content/${doc.workgroup}/Docs/GoogleDocs/${doc.google_id}/doc-with-comments.md`;
        }
        let currentSHA = null;
        try {
        ...
        await octokit.repos.createOrUpdateFileContents({
          ...
          repo: "SingularityNET-Archive",
          path,
          message: `Update document text and comments for ${doc.google_id}${date ? ` on ${date}` : ''}`,
  ...
  export async function handler(event, context) {
    try {
      ...
      if (!test) {
        // Process each document
        await commitDocumentTextAndCommentsToGitHub(docsToCommit, date);
      }
      return {
        statusCode: 200,
